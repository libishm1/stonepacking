...
export function createUI(root, initial, onGenerate, onExport) {
    ...
    form.append(title, renderField("Count", countInput), renderField("Min scale", minScaleInput), renderField("Max scale", maxScaleInput), renderField("Spacing", spacingInput), renderField("Target fill (0-1)", targetFillInput), renderField("Container preset", presetSelect), renderField("Width", widthInput), renderField("Height", heightInput), renderField("Depth", depthInput), renderField("Max attempts", attemptsInput), renderField("Packing passes", passesInput), renderField("Auto-size container", autoContainerToggle), renderField("Shape", shapeSelect), renderField("Enable physics", physicsToggle), buildButtonRow(onExport));
    ...
    function readState() {
        ...
        const state = {
            ...
            shapeMode: shapeSelect.value || initial.shapeMode
        };
...
function renderField(labelText, input) {
    const label = document.createElement("label");
    label.textContent = labelText;
    label.appendChild(input);
    return label;
}
function buildNumberInput(name, value, min, step = 0.1, max) {
...
function buildShapeSelect(current) {
    const select = document.createElement("select");
    select.name = "shape";
    [
...
function buildButtonRow(onExport) {
...
function applyPreset(preset, inputs) {
    if (preset === "custom") {
        return;
    }
