...
export function scorePlacement(position: Vector3, radius: number, spacing: number, container: Container, neighbors: SupportSample[]): number {
...
function proximityScore(position: Vector3, radius: number, spacing: number, neighbors: SupportSample[]): number {
...
function heightScore(position: Vector3, container: Container): number {
  const heightAboveFloor = position.y + container.height / 2;
  return -heightAboveFloor;
}

function centeringScore(position: Vector3, radius: number, spacing: number, container: Container): number {
