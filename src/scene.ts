...
import { Container, PackedObject } from "./types";

export interface SceneController {
  setContainer(container: Container): void;
  setShapeMode(mode: "dodeca" | "brick"): void;
  renderObjects(objects: PackedObject[]): void;
  updateInstances(objects: PackedObject[]): void;
  onFrame(handler: (delta: number) => void): void;
  exportGLTF(filename: string): void;
  dispose(): void;
}

export function createScene(target: HTMLElement, initialContainer: Container): SceneController {
  ...
  let instanceCount = 0;
  let frameHandler: ((delta: number) => void) | null = null;
  let shapeMode: "dodeca" | "brick" = "dodeca";
  ...
  return {
    setContainer(container: Container) {
      ...
      (containerHelper.material as THREE.Material).dispose();

      containerHelper = buildContainerHelper(container);
      scene.add(containerHelper);
      repositionCamera(camera, controls, container);
    },
    setShapeMode(mode: "dodeca" | "brick") {
      shapeMode = mode;
    },
    renderObjects(objects: PackedObject[]) {
    ...
    updateInstances(objects: PackedObject[]) {
      if (!shapeMesh) {
    ...
    onFrame(handler: (delta: number) => void) {
      frameHandler = handler;
    },
    exportGLTF(filename: string) {
      exporter.parse(
        scene,
        (gltf) => {
...
function createRenderer(target: HTMLElement): THREE.WebGLRenderer {
...
function createCamera(target: HTMLElement): THREE.PerspectiveCamera {
...
function createControls(camera: THREE.PerspectiveCamera, renderer: THREE.WebGLRenderer): OrbitControls {
...
function buildContainerHelper(container: Container): THREE.LineSegments {
...
function startRenderLoop(
  ...
  handlerFactory: () => ((delta: number) => void) | null
): void {
...
function resizeToTarget(renderer: THREE.WebGLRenderer, camera: THREE.PerspectiveCamera, target: HTMLElement): void {
...
function repositionCamera(camera: THREE.PerspectiveCamera, controls: OrbitControls, container: Container): void {
...
function disposeSingle(mesh: THREE.InstancedMesh | null): void {
  if (!mesh) {
    return;
  }
...
function buildBrickGeometry(): THREE.BufferGeometry {
...
function composeMatrix(matrix: THREE.Matrix4, object: PackedObject): void {
