import { Container, PackedObject, Vector3 } from "./types";
import { distanceSquared } from "./utils";

interface PhysicsBody extends PackedObject {
  velocity: Vector3;
  mass: number;
}
...
export class PhysicsEngine {
  ...
  constructor(objects: PackedObject[], container: Container, spacing: number) {
...
function applyGravity(bodies: PhysicsBody[], dt: number): void {
  for (const body of bodies) {
    body.velocity.y += GRAVITY * dt;
  }
}

function integrate(bodies: PhysicsBody[], dt: number): void {
  for (const body of bodies) {
...
function resolveContainerCollisions(bodies: PhysicsBody[], container: Container, spacing: number): void {
  const half = {
    x: container.width / 2,
...
function resolveBodyCollisions(bodies: PhysicsBody[], cellSize: number, spacing: number): void {
...
function resolvePair(a: PhysicsBody, b: PhysicsBody, spacing: number): void {
...
function dampVelocities(bodies: PhysicsBody[]): void {
  for (const body of bodies) {
...
function computeCellSize(objects: PackedObject[], spacing: number): number {
  const maxRadius = objects.reduce((max, obj) => Math.max(max, obj.radius), 0);
  return Math.max(maxRadius * 2 + spacing, 0.5);
}

function indexFromPosition(position: Vector3, cellSize: number): GridIndex {
  return {
...
function cellKey(index: GridIndex): string {
