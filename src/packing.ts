...
import { Container, PackedObject, PackingParams, PlacementCandidate, SupportSample, Vector3 } from "./types";
...
export function deriveContainer(params: PackingParams): Container {
...
export function packObjects(params: PackingParams, seeded: PackedObject[] = []): PackedObject[] {
...
function placeStone(params: PackingParams, masonry: MasonryConfig, grid: SpatialGrid, placed: PackedObject[]): PackedObject | null {
...
function searchPlacementForRotation(
  params: PackingParams,
  masonry: MasonryConfig,
  grid: SpatialGrid,
  supportList: SupportSample[],
  ...
  for (let attempt = 0; attempt < params.maxAttempts; attempt++) {
    ...
    const position: Vector3 = { x: snapped2d.x, y, z: snapped2d.z };
...
function samplingBounds(container: Container, radius: number, spacing: number): { minX: number; maxX: number; minZ: number; maxZ: number } {
...
function randomXZ(bounds: { minX: number; maxX: number; minZ: number; maxZ: number }): { x: number; z: number } {
  return {
...
function normalizeParams(params: PackingParams): PackingParams {
...
function computeCellSize(params: PackingParams): number {
  const averageScale = (params.minScale + params.maxScale) / 2;
  const averageRadius = BASE_RADIUS * averageScale;
...
function toSupportSample(object: PackedObject): SupportSample {
  return { position: object.position, radius: object.radius };
}

export function computeFillRatio(objects: PackedObject[], container: Container): number {
...
function buildMasonryConfig(params: PackingParams): MasonryConfig {
  const averageScale = (params.minScale + params.maxScale) / 2;
  const averageRadius = BASE_RADIUS * averageScale;
...
function snapToCourse(height: number, config: MasonryConfig): number {
...
function computeLayerIndex(y: number, config: MasonryConfig): number {
...
function adjustMasonryAfterFirst(config: MasonryConfig, firstRadius: number, params: PackingParams): MasonryConfig {
...
function computeCourseHeightFromRadius(radius: number, spacing: number): number {
